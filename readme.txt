=== Footnotes ===
Contributors: markhowellsmead, yingling017, twinpictures
...
Footnotes lets you quickly and easily add footnotes throughout your WordPress posts using a simple shortcode in the text editor.

== Description ==

Footnotes lets you add footnotes throughout your WordPress posts by using the shortcode [note]Footnote content.[/note]. Footnotes will automatically add the number of the footnote where the shortcode was entered and add the full footnote text to the bottom of your post in an ordered list with a corresponding number.
...
1. Upload the 'shp-footnotes' folder to the '/wp-content/plugins/' directory.
...
Simply use the shortcode [note]Footnote content goes here[/note] and Footnotes will enter numeric footnotes into your post that open on hover and take the user to the footnote at the bottom of the page on click.
...
Just use the `shpefn_footnote_label` filter in your functions.php to edit the output.

<pre>
<code>function shpefn_change_label_markup( $output, $label ) {
    return '<h5>' . $label . '</h5>';
}
add_filter( 'shpefn_footnote_label', 'shpefn_change_label_markup', 10, 2 );</code>
</pre>
...
<pre>
<code>function shpefn_deregister_scripts() {
	wp_deregister_style( 'qtipstyles' );
	wp_deregister_script( 'imagesloaded' );
	wp_deregister_script( 'qtip' );
	wp_deregister_script( 'qtipcall' );
}
add_action( 'wp_enqueue_scripts', 'shpefn_deregister_scripts' );</code>
</pre>
...
= 2.0.0 =
* Take over to Say Hello version from original.
* Reinstate [note] shortcode and add [notiz] for German-speaking users.

= 1.1.4 =
* Fixing typo on FAQ page

= 1.1.3 =
* Adding div.shp-footnote-title into $footnote_label for filtering with `shpefn_footnote_label`
...
* Fixed undefined index of $shpefn_output when not using Footnotes label

= 1.1.1 =
* Added `shpefn_footnote_list_output` filter for editing entire Footnote output after content
...
* Added second option for footnotes using `[note]` to phase out non-prefixed `[note]`
* Added `shp_footnote_label` hook to filter footnote labels
...
* New setting to hide the footnotes after content in Settings > Footnotes
...
= 1.0.6 =
* Added the ability to insert a title above the footnote section at the bottom of the post content. This is controlled in the Footnotes Settings page that can be found under Settings in the WordPress Dashboard.
